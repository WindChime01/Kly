...
function jinbi($member,$money,$desc,$jj,$type,$bitype,$username = NULL){

	$data = array();
	if($bitype == 'ipfs') {
		$jinbidetail = M('jinbidetail');
    	$jinbi = M('member')->where(array('username'=>$member))->getField('ipfs');
	} elseif($bitype == 'yuanqi') {
		$jinbidetail = M('yuanqidetail');
    	$jinbi = M('member')->where(array('username'=>$member))->getField('yuanqi');
	} elseif($bitype == 'my_sl') {
		$jinbidetail = M('jinbidetail');
    	$jinbi = M('member')->where(array('username'=>$member))->getField('my_sl');
	} elseif($bitype == 'fil') {
		$jinbidetail = M('fildetail');
    	$jinbi = M('member')->where(array('username'=>$member))->getField('fil');
	} else{
		return false;
	}
...
function juheSendSMS($mobile, $code,$session_id){
    #------测试---用别的服务器发送短信-
    $url = 'http://juhesms.weiyinstudio.com/auto/api/xybjuheSendSMSApi';
    $params = [
        'mobile'    => $mobile,
        'code'      => $code
    ];
...
// 	$qycode = $code;
// 	$params = array(
// 		'key'   => 'e2a6c972154c9fa24ca68e35f65ffb67', //您申请的APPKEY
// 		'mobile'    => $mobile, //接受短信的用户手机号码
// 		'tpl_id'    => '215661', //您申请的短信模板ID，根据实际情况修改
...
function ccurlOpen($url, $config = array()){
	...
	function write_log($account , $type , $desc){
		  ...
		  //  $data['logiplocal'] = $location['country'].$location['area']; // 所在国家或者地区
		    $location = ipToArea(get_client_ip()); // 获取某个IP地址所在的位置
		    $data['logiplocal'] = $location; // 所在国家或者地区
...
function getRandChar(){
	$length = 6;
    $str = null;
    $strPol = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";//大小写字母以及数字
    $max = strlen($strPol)-1;
...
function ejmm($pwd,$id){
	$getpwd = M("member")->where(array("id"=>$id))->getField("password2");
	if (md5($pwd) != $getpwd){
		return 0;
	}else{
		return 1;
...
function withdrawal_status($status){
	switch ($status) {
		case 0:
			return "未确认";
			break;
...
// 无限下级
function m_GetTeamMember($members, $mid)
{
    $Teams = array(); //最终结果
    $Teams[] = strval($mid);
    $array = array();

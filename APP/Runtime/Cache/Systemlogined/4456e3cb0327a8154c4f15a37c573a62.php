	...
	function intoApply($id) {
						...
						<form id="table-searchbar" method="GET" class="form-inline well well-small">
							...
							<div class="row-fluid" style="margin-bottom: 20px">&nbsp;&nbsp;
								<select class="span3" name="type" style="width: 100px;">
									...
									<option value="1" <?php if($type == 1): ?>selected="selected"<?php endif; ?>>姓名</option>

									<option value="2" <?php if($type == 2): ?>selected="selected"<?php endif; ?>>手机号</option>

									<!-- <option value="3" <?php if($type == 3): ?>selected="selected"<?php endif; ?>>级别</option> -->

									<option value="4" <?php if($type == 4): ?>selected="selected"<?php endif; ?>>来源</option>
		...
		<script type="text/javascript">
			...
			function change_reward_status(){
			    if(confirm("确认要进行分配吗?")){
    				$.post("<?php echo U(GROUP_NAME .'/LevelReward/change_reward_status2');?>","",function(e){
			...
			function clear1(){
				...
				if(confirm("确认要导出所查询信息吗?")){
					$('#table-searchbar').attr('action',"<?php echo U(GROUP_NAME .'/LevelReward/excel2?type=1');?>").submit();
			...
			function clear2(){
				...
				if(confirm("确认要导出全部信息吗?")){
					$('#table-searchbar').attr('action',"<?php echo U(GROUP_NAME .'/LevelReward/excel2?type=2');?>").submit();
					$('#table-searchbar').removeAttr('action');
				}
			}

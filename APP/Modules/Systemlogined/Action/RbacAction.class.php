<?php
	...
	Class RbacAction extends CommonAction{
		...
		public function editNode(){
			...
			$node = M('node')->where(array('id'=>I('nid','','intval')))->find();

			$zi = M("node")->where(array("pid"=>I('nid','','intval')))->select();
		...
		public function editNodeHandle(){
			...
			foreach ($data['childname'] as $key => $value) {
				$edit = array(
					"name"		=> $data['childname'][$key],
					"title"		=> $data['childtitle'][$key],
					"status"	=> 1,
					"sort"		=> 1,
					"pid"		=> $nid,
					"level"		=> 4
				...
				if ($data['childid'][$key] != "") {
					$ret = M("node")->where(array("id"=>$data['childid'][$key]))->save($edit);
				}else{
					$ret = M("node")->add($edit);
				}
			}

			if (M('node')->where(array('id'=>$nid))->save($data) || $ret) {
		...
		public function editRole(){
			...
			$role = M('role')->where(array('id'=>I('rid')))->find();
			$role['auth'] = json_decode($role['auth'],true);

			$nodes = M("node")->where(array("level"=>2))->select();
			foreach ($nodes as $key => $val) {
				$role['auth'][$val['name']] = json_decode($role['auth'][$val['name']],true);
				$nodes[$key]['children'] = M("node")->where(array("level"=>3,'pid'=>$val['id']))->select();
			}
			// dump($role['auth']);die;
			$this->assign('role',$role);
			// dump($nodes);die;
			$this->assign("nodess",$nodes);
		...
		public function editRoleHandle(){
			...
			$nodes = M("node")->where(array("level"=>2))->select();
			foreach ($nodes as $key => $val) {
				$data['auth'][$val['name']] = json_encode($data[$val['name']]);
			}
